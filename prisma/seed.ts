...
// Contoh template kuesioner (silakan ganti / tambah sesuai kebutuhan)
// Struktur questions & resultTiers mengikuti asumsi implementasi endpoint.
...
async function main() {
  ...
  if (existingDoctor) {
    console.log('Doctor already exists, skip creating doctor. Using doctorId =', existingDoctor.id);
  ...
  for (const tpl of questionnaireTemplates) {
    const created = await prisma.questionnaireTemplate.create({
      data: {
        ...
        jenis_kuesioner: tpl.jenis_kuesioner as any,
        doctorId,
        questions: tpl.questions as any,
        resultTiers: tpl.resultTiers as any
      }
    });
